...
Subagents are defined in Markdown files with YAML frontmatter. You can [create them manually](https://code.claude.com/docs/en/sub-agents#write-subagent-files) or use the `/agents` command.
...
Subagents can use any of Claude Code’s [internal tools](https://code.claude.com/docs/en/settings#tools-available-to-claude). By default, subagents inherit all tools from the main conversation, including MCP tools.
...
If a background subagent fails due to missing permissions, you can [resume it](https://code.claude.com/docs/en/sub-agents#resume-subagents) in the foreground to retry with interactive prompts.
