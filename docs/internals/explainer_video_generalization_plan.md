...
| 1 | **IN PROGRESS** — parallel capture landed (0.8.0; byte-identical verified, ~1.0x on a 4-core software-GL box — see roadmap item 5). Canvas2D template + easing personalities + first STYLE block landed (0.9.0; smoke green on both backends 4/4 runs, 3D output byte-identical after the kit addition; two composition bugs found by frame review — fp-residue gate leak, luminance-blind label ink — and one smoke.js sampling race found because its symptoms masqueraded as scene findings, all fixed). Style packs + kernel parity landed (0.10.0): three packs in `references/styles/`, blueprint swap verified categorically different on unchanged beats (and caught a polarity bug in `contrastOn`); shared kit is a marked KERNEL block, byte-identical across templates, smoke.js hard-fails on drift (positive control run); 3D STYLE split done, byte-identical after refactor. Remaining: the 2D proving film (gate), then any residual kernel extraction decisions. |
| 2-4 | not started |
