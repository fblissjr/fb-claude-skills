...
| 2 | **IN PROGRESS — spike gate MET** (0.12.0). `examples/toybot-walk.html`: cel + gradient-map bands (hemisphere lowered — it washes toon bands, measured), inverted-hull outlines riding the IK pivots, analytic two-bone IK with a START-anchored plant grid (origin-anchored shipped legs-horizontal; sheet caught it), rack-focus DoF from live camera distances, bloom budgeted to the payoff, `vendor` now bundles the composer classes. Post chain passes smoke byte-determinism, source + bundled. Roadmap item 10 closed by this example. Remaining in phase: procedural-sky IBL, matcap/physical packs, InstancedMesh fields + lathe/extrude geometry, quality tiers (deferred until a film hurts — rule pre-decided: determinism + shipping at FINAL). |
| 3-4 | not started |
