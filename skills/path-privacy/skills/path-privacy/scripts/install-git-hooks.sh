...
doctor_repo() {
  ...
  for h in pre-commit commit-msg; do
    ...
    if [ "$ver" != "$WRAPPER_VERSION" ]; then
      if pp_template_is_newer "$ver" "$WRAPPER_VERSION"; then
...
if [ "$DOCTOR" -eq 1 ]; then
  echo "path-privacy doctor -- plugin $PLUGIN_VERSION, wrapper template $WRAPPER_VERSION"
  echo ""
...
# Compare TEMPLATE stamp to TEMPLATE stamp. Comparing this stamp against the
# plugin's own version differs on every unrelated release, so the notice fired on
# every commit forever and taught people to scroll past it.
RESOLVED_VERSION=\$(sed -n 's/^WRAPPER_TEMPLATE_VERSION=\\(.*\\)\$/t\\1/p' \\
                   "\$RESOLVED_ROOT/skills/path-privacy/scripts/install-git-hooks.sh" 2>/dev/null | head -1)
if [ -n "\$RESOLVED_VERSION" ] && [ "\$RESOLVED_VERSION" != "\$WRAPPER_VERSION" ]; then
  if pp_template_is_newer "\$WRAPPER_VERSION" "\$RESOLVED_VERSION"; then
    echo "path-privacy: this \$HOOK_NAME hook (\$WRAPPER_VERSION) is NEWER than the resolved plugin (\$RESOLVED_VERSION)." >&2
    echo "  The gate is fine; the plugin is behind. Nothing to do -- do NOT reinstall, which" >&2
    echo "  would regenerate this hook from the older plugin." >&2
  else
    echo "path-privacy: this \$HOOK_NAME hook (\$WRAPPER_VERSION) predates the plugin's wrapper template (\$RESOLVED_VERSION)." >&2
    echo "  Open a Claude Code session in this repo and it refreshes itself; no command to run." >&2
  fi
fi
