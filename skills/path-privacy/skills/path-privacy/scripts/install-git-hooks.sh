...
doctor_repo() {
  ...
  for h in pre-commit commit-msg; do
    ...
    [ "$ver" != "$WRAPPER_VERSION" ] && DOCTOR_RC=1
    printf '  %-11s ver=%-24s %-11s %s\n' "$h" "$ver" "$openness" "$frozen"
  done
  return 0
}
...
# Direction decides the remedy, so this cannot be a string comparison. A
# wrapper installed from a source checkout is NEWER than a lagging installed
# plugin; telling that case to re-run the installer would regenerate this hook
# from the OLDER plugin and silently downgrade a working gate.
if [ -n "\$RESOLVED_VERSION" ] && [ "\$RESOLVED_VERSION" != "\$WRAPPER_VERSION" ]; then
  if [ "\$(printf '%s\\n%s\\n' "\$WRAPPER_VERSION" "\$RESOLVED_VERSION" | sort -V | head -1)" = "\$WRAPPER_VERSION" ]; then
