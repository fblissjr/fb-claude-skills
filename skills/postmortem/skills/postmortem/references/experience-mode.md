...
Read this when the postmortem's subject is **the system the work was done
with**, not the work itself. Session, span, and feature modes ask *what did we
build and what did it teach us*. Experience mode asks *what was it like to
build with this thing, and what should its developers change*.

The audience is different, and that is the whole reason it is a separate mode:
the reader is someone who maintains the subject and has never seen your
repository. Everything about the format follows from that.
...
If you were changing it, this is not experience mode — that is a feature
postmortem of your own work, and the subject's developers are you.

## The one hard problem this mode has

Every other mode reads evidence that outlives the run: commits, files, logs.
Experience mode's evidence is *friction*, and friction is the thing that
...
exactly as it does everywhere else, and in this mode it bites hardest:
...
## Citations that work in this mode
...
5. **A figure** — the produced output next to the intended output, when the
   finding is about something visible. See `references/visual-evidence.md`.
...
Experience mode replaces the five sections in `references/report-format.md`
with these six. Everything else in that file still holds: a few sentences per
finding, "Nothing." is valid output, and never invent an item to fill a frame.
...
The structural twin of the escapes section in the standard format. For every
...
This is the highest-signal section in the mode, because each row is a
...
The routing table in `references/report-format.md` assumes the findings are
about this repo, and in this mode most of them are not. Adapt it:
...
real command output — the exact material this mode is built on. Repo-relative
...
`references/visual-evidence.md` states the capture discipline.
