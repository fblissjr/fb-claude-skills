...
Two standing prompts for long, mostly unattended runs, packaged so they are
invoked by name, versioned, and kept in one place.
...
Both are user-invoked only (`disable-model-invocation: true`): a run spends hours
and many subagents, so it starts only when someone types it. That also keeps
both out of the always-loaded skill listing.
