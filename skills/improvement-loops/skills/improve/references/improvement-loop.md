...
- Run budget: about 4 hours. See `common.md` <budget>.
- Landing: a branch `improve/<run id>` from the base commit, entered as `common.md` <worktrees> describes, one commit per change. State each commit's evidence as a relationship plus a pointer to its scoreboard rows. Don't merge or push.
...
<each_run>
1. Orient. Start the run with `loop_state.py start`, registering this prompt's done list, then enter the branch worktree (`common.md` <loop_state>, <worktrees>). In your worktree, read AGENTS.md and the status, plans and backlog it points to. In the loop state, read the ledger, the scoreboard, the bookmarks and my answers. Read the git log since each bookmark, and check pinned dependencies' release notes since theirs. Check that every file, command and behaviour this prompt and AGENTS.md name still holds on the base commit, and list any that don't under "Needs from me".
...
3. Find. Launch the reviewers as `common.md` <reviewers> describes, one per lens. Skip a lens whose bookmark shows that nothing it covers has changed, and move each reviewed lens's bookmark.
4. Improve.
   ...
   g. Commit, then append to the ledger and the scoreboard through `loop_state.py`, and mark any done item this met. Put the updated table in the same message as your next action.
5. Review. Relaunch the reviewers in review mode on the run's diff (`common.md` <reviewers>). Fix whatever blocks the merge, and log the rest.
...
7. Report. Build the page, append your session-log section, and tidy the ledger if `loop_state.py can-tidy` allows it.
8. Grade. Give the loop-grader agent the done list, the run record, the report and the north star (`common.md` <reviewers>). If something is unmet, fix it, then repeat from step 6. When nothing is, finish the run with `loop_state.py finish --status done`.
</each_run>
