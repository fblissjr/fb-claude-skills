...
metadata:
  verified_against: "heylookitsanllm 1.79.42"
---
...
will succeed. It is derived from the model's **registry entry**, while the
refusal is decided from the **model as loaded** — so on the MLX path a
variant whose entry still declares vision advertises `vision` and is then
refused, as is audio sent to any MLX model. Treat that refusal as a real
answer, and note it arrives in two shapes: non-streaming it is a 400, but on
a stream the guard fires at the first token, after the headers have flushed,
so it arrives in-band as an `error` event typed `invalid_request_error`. A
client that treats gating as sufficient renders it as a hang or as
assistant text. On the **gguf path heylook adds no capability guard** — it
forwards the block to `llama-server`, and the outcome splits on what that
subprocess does. Its own 400 is normalized into the same refusal, so it
reaches you in the two shapes above and needs no extra handling; accepting
the block and ignoring it is the silent case, a 200 describing an image the
model never used. Which branch you land on is the model's GGUF/mmproj
packaging, not heylook.
