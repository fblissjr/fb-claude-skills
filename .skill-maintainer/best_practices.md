...
- [ ] What a **command** hook does when it times out is NOT documented. Do not infer it from the HTTP-hook section (which fails open but states it differs from command hooks) or from the Agent SDK callback section (where a `PreToolUse` timeout fails closed) -- those two disagree
