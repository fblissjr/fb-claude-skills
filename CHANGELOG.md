...
### changed
- **`postmortem` 0.9.0 → 0.10.0 — the family gets a shared layer, after an audit found six of eight defects were one cause.** The plugin grew from one skill to five, and three cross-cutting concerns — filing, rendering, verification — were each re-answered by every skill that needed them. `filing.md` was the tell: a plugin-wide concern living inside the `postmortem` skill, so `postmortem-index` reached across into a sibling's references and `test-audit` simply never answered where its output went.

  **A plugin-level `references/` now holds `filing.md` and `verification.md`.** The rationale is narrow and worth stating, because it is not the usual one: there is no import mechanism in a plugin made of prose — a file is shared only because several skills name its path — so **the path is the only signal a future editor gets about who depends on a file**, and a directory location is a hint rather than enforcement. The specimen justifying the move was produced by 0.9.0 itself: adding `version` and `task` required hand-editing two enumerations of the frontmatter field set, with nothing connecting them. `filing.md`'s table is now the only enumeration, and `postmortem-index` reads it instead of keeping a list.

  **The needle rule had drifted into four wordings** across `control-builder` step 4, `adversarial-verify` step 2, `test-audit`'s standing cautions, and `control-audit`'s live-fire protocol. One home now, cited at each dispatch point; skills keep a one-line posture claim with no rationale. Note what the old arrangement implied: `adversarial-verify` pointed callers at the *agent's* step 4 as canonical, making an agent definition — a system prompt for a different context — the documentation home for a rule two skills need before any dispatch happens. The agent now cites the shared file too, and states the boundary it could not state before: it returns three verdicts, and `vacuous` is deliberately not its to issue, because it does not grade its own needle.

  **Deliberately not extracted: the HTML palette.** Both templates carry an identical twelve-declaration `:root` block, and the obvious move is to share it. Measured, that is ~12 lines, and `html-render.md`'s template is marked "embed verbatim" precisely so it emits one self-contained file — extraction would make every render an assembly step, a daily cost to remove a small duplicate with no third consumer pending. So the pair stays, and `skill-maintain` 0.27.0 → 0.28.0 adds `repo/postmortem palette pair`, which extracts `--name: value` declarations from both templates and compares them in order (`var(--name)` usages have no trailing colon, so the chart rules that reference the palette cannot match). Born green as a pin over existing behaviour, so mutation-proven at birth per the TDD rule: one hex changed in the index template → `FAIL ... drift: [('accent', '#7a5c2e'), ('accent', '#AA0000')]`, reverted → PASS. That converts an undisclosed copy into a watched pair, which is invariant 1b's second question answered rather than dodged.

### fixed
- **An annotation left its `.html` sibling asserting the uncorrected finding.** `filing.md` said annotate the markdown; `html-render.md:14` says that if the two disagree the rendering is wrong by definition; nothing connected them, so the normal correction path manufactured a state the skill defines as wrong, silently, on the copy most likely to have been sent to someone. Annotating now re-renders every sibling rendering — distinct from the unsupported "render an arbitrary old file" case, since an annotating run has just read and edited the file — and deletes any rendering it cannot faithfully re-render, because a missing rendering is recoverable from the markdown and a stale one is not. Every rendering also gains a provenance footer naming its source file and render date, so a hand-edit outside the skill leaves something a reader can notice. The gap was documented and then dropped: `postmortem_output_formats.md:127` listed it under "constraints that must survive" and the resolved-questions list below it never resolved it.
- **`test-audit` never said where its output goes**, sitting between two siblings explicit in opposite directions ("always a file" / "persists nothing"), while the README asserted it was "markdown only" — a format decision about a file the skill never told anyone to write. It now files via the shared ladder with mode token `audit`. The distinction from `control-audit` is principled rather than arbitrary: a controls census is a snapshot of live config that goes stale the moment a hook changes, so a stored copy competes with the code; test verdicts are judgments about intent, expensive to reconstruct and not rotting on that clock. The value of auditing a suite twice is the diff against last time, and there is no diff without a file. This also gave `filing.md` its third consumer, which is what makes the extraction above load-bearing rather than speculative.
- **`--out` meant two different things inside `postmortem-index`** — the filing ladder's rung 1 (where postmortems are read) and the page's write location, both reachable from one skill and both supported by the text. Split: `--from=<dir>` reads, `--out=<dir>` writes, so `--out` keeps one meaning family-wide ("where the deliverable goes").
- **The index is an HTML page that linked only to markdown.** Opened in a browser, every entry was a download or raw text while a rendered sibling might sit beside it. It now links a rendering when one is confirmed to exist, with the markdown as a secondary link, and falls back silently when there is none — renderings are derived and may be absent at any time.
- **`adversarial-verify` carried a deferred TODO in shipped prose** ("`claim-audit` gains its explicit pointer here at its next content release"). Accurate but unwatched — nothing fires when `claim-audit` releases. Replaced with a statement of the current relationship.
- Disclosed rather than fixed: files at a plugin root sit outside `measure_tokens`, which rglobs a skill directory, so `ref_tokens` now undercounts the reference surface of every skill reading a shared file. The budget gate is on SKILL.md only, so nothing is bypassed and no threshold moved — but the informational number is no longer complete, and that is stated in the plugin README rather than left to be discovered.

## 1.14.0

### added
- **`postmortem` 0.8.1 → 0.9.0 — a fourth mode, `experience`, and `--visuals`.** Both generalize a prompt that was being pasted by hand into other repos to get a developer-facing writeup of what it was like to build with a tool, with screenshots of the artifact it produced. Two things needed generalizing out of it: the subject (a compiler, in the specimen — now any system with a surface you had to learn: framework, API, CLI, harness, agent, skill, model) and the media (frames of the rendered scene — now any figure a finding is about).

  **Experience mode** inverts the audience: the reader maintains the subject and has never seen this repository, and every format decision follows from that. Six sections instead of five. Two reuse the standard spine structurally rather than by name — the deviations table becomes **Expected | Actual | What led me to expect it**, where the third column names the surface that misled and is therefore the thing to fix, and the test-escapes section becomes escapes over guidance and instrumentation with a three-way verdict (absent / present-but-not-found / present-but-misleading) rather than the tests' two-way missing-or-blind. The genuinely new one is **Built outside the system**, each row a capability request that already ships with a working reference implementation, split into workarounds (should stop existing) and legitimate extensions.

  The mode's hard problem is that its evidence is friction, which is the first thing a successful session overwrites — by the time the work lands, four wrong turns have been overwritten by the one path that worked and "the docs were a bit unclear" is what is left. So the grounding rule is restated at the grain this mode needs: *"I was confused" is not a finding; the turn where you were confused, what you did next, and what it cost is*, reconstructed from the session trace rather than from memory of it, with gaps declared rather than filled in plausibly. Two rules decide whether the output is usable at all: rank by cost rather than annoyance (a finding with no cost attached is a preference, and preferences from one user are noise), and **separate "it is missing" from "I did not find it"** — the second is still a finding, but a discoverability one whose fix is unrelated, and misfiling it sends a developer to build something they already shipped. Frontmatter gains `version` (the build as it was used — answers *does this still apply*) and `task` (answers *does this apply to me*, and is not recoverable from `summary`, which carries the conclusion rather than the setting).

  **`--visuals`** applies the grounding rule in both directions: *no finding, no figure* — a figure with no finding is the visual form of the generic advice the skill already bans — and, where a claim is about something visible, prose is a lossy citation. Those two bound the figure set from both sides and bound its size as a side effect. Charts resolve the same way the whole skill does: **the numbers live in a table in the markdown and the chart is a rendering of that table**, so a chart cannot contradict the record for the same reason the HTML cannot, and charts write no files at all. They are hand-authored inline SVG drawn with the page's custom properties — the no-JavaScript rule means anything rendering client-side renders nothing, and a chart that is a white rectangle in dark mode is not readable either. Captured media goes in a sidecar directory with the postmortem's stem; the markdown references it relatively and the HTML inlines it as `data:` URIs, which is the self-containment constraint doing its job rather than an exception to it — a relative image link breaks "send this one file to someone" as thoroughly as a CDN link does. That directory is why this is a flag rather than a judgment call: filing never silently creates a directory in a layout the repo did not choose, and the flag is the consent. `--visuals` implies `--html`, since a figure set whose only rendering is relative links is half a deliverable.

  One gap named while writing it, and it is a real hole in this repo's own controls rather than a property of the skill: **path-privacy's hooks read text and cannot see inside a PNG.** A screenshot catching a terminal title bar, an editor sidebar, or a window title carries absolute paths and usernames past every check here — and an experience postmortem is the one postmortem written specifically to be sent elsewhere. Redaction is therefore specified as part of capturing rather than a review step afterwards, with cropping named as the first lever because it satisfies the requirement and improves the figure at the same time.

  `postmortem-index` reads the two new fields and renders `version` beside the mode badge; both join `data-search`, since a term a reader can see but cannot filter by reads as a broken filter. No index changes were needed for the mode token itself — `.mode` was already one class over an open set, which is why a fourth mode cost nothing there.
