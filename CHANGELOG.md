...
### fixed
- **The staleness check added in 1.41.0 had four defects, and the guard meant to make it harmless was the worst of them.** `source_version` caught `(OSError, KeyError, ValueError)`, which covers unparseable TOML but not a well-formed file whose `project` key is a string — that raises `TypeError`, and because the call sits ahead of dispatch it crashed **every** `skill-maintain` command. The test written to prove the guarantee only fed malformed TOML, so it never reached the path. A non-string `version` was returned unconverted, making the comparison permanently unequal so the warning fired forever and the reinstall it recommends never cleared it — a check that cannot be satisfied, which is the wallpaper failure its own sibling test forbids. `init` was exempt because the call sat below that branch's `sys.exit(0)`, and `init` is where a stale build does the most damage: `install_pre_commit_hook` writes the template out of the running build, so an old binary scaffolds an outdated hook silently. And the source root was the process CWD, so any invocation from a subdirectory disabled the check without saying so.
- **Root resolution now walks up from the working directory, and `--dir` is used only when it actually vendors a copy.** The first fix took `--dir` at face value, which broke `init --dir /fresh/repo` — that flag names the repo being acted on, not where this tool's source lives, so the check went silent again on exactly the command it had just been extended to cover. Caught by running the binary; the unit test missed it because it stubs `staleness_warning` and so never exercises the resolver. Deliberately not read from uv's tool receipt: that records the tree the binary was *built* from, where the question here is whether the build matches the source you are presently reading.
- **A false claim shipped in `best_practices.md`, in the item about not trusting unqualified claims.** "A failed shell glob and a clean run exit identically — both 1 when bare" is zsh-only. Measured: zsh 1/1, **bash 2/1**, so bash distinguishes them and a reader on bash was being told to skip an exit-code check that would have caught the failure. The piped clause holds in both shells and carries the real weight. Same file, "this environment's `grep`" shipped to every installed repo, where "this environment" is someone else's machine — the NUL-byte blindness is a property of a `ugrep -I` shim, and GNU and BSD grep print "Binary file X matches". Both now scoped.
- **The `freshness` retirement had missed twenty live references across nine files.** `quality/SKILL.md` still listed `freshness` as a runnable command, which produces `Unknown command`. Both marketplace descriptions, both plugin manifests, the dashboard's `package.json`, three READMEs and an eval fixture still advertised freshness as a capability. These are the widest-reach copies — the descriptions load into every session's listing — and the commit that missed them is the one that added "the copy with the widest reach is the one the editing loop never looks at". The original sweep took its list from the fix, which is the rule, but the fix had two vocabularies (`metadata.last_verified`, bare `freshness`) and only one was swept.
- `wire_reference.md` gave two incompatible scopes for the unreachable `error` stop reason — the header said "before 1.79.39" while the body said 1.79.39 declared it, so a reader pinned to 1.79.39 would read the header and conclude it did not apply. Now "every version through 1.79.39" in both places.
- The Pillow harness no longer claims more than it is. Nothing runs it automatically — no CI, and `skill-maintain test` carries no pytest dependency by design — so `client_recipes.md` says it is a check you can re-run rather than a gate that notices drift. `MAX_EDGE` is read out of the extracted recipe instead of being hand-restated, the extraction rule has one home instead of two, and the recipe is compiled once rather than eight times. Verified both ways: reintroducing the ordering bug still reddens three cases, and moving the recipe's `MAX_EDGE` to 512 now passes rather than failing with a misattributed message.
...
### added
- **`best_practices.md` gains "one claim in several places", the lesson five same-day incidents were all instances of.** Per-path tests cannot see cross-path divergence: where two paths must agree, tests asserting each path's behaviour are individually correct and collectively blind, and the specimen is a `stop_reason` passthrough that existed in two modules where 1,700 tests stayed green while the routes disagreed. Assert that the paths agree, not that each is right. The existing "hand-copied constant is a defect with a delay" framing does not reach a copied *shape* — a duplicated description in a documentary model that no assertion reads, which drifts in the same commit that fixes the other copy. And the control that actually finds these: **sweep the generated artifact, not the source.** Every widest-reach copy lands in something rendered, so one pass over that for strings known to be false catches route descriptions, headers, examples and endpoint maps at once, where sweeping the source catches whichever file you thought to open. That control is the heylook session's, and it found two more instances within minutes of being adopted. The section closes on why: the copy with the widest reach is the one the editing loop never looks at — a skill `description` sits above the prose being edited, making it simultaneously the most-read instance of a claim and the least-reread.

### fixed
- **`skill-maintain` now says when the build you are running is not the source you are reading.** There are two installs with that name: the `uv tool install` on PATH and the editable workspace install `uv run` resolves. The tool install's receipt points at `tools/skill-maintainer`, so it reads as tracking the source while pinned at whatever it was built from — found today at **v0.18.1 against a 0.34.0 source**, still offering the `freshness` subcommand deleted the same day. That is not cosmetic: the skills shipped from this repo instruct the *bare* command, so the stale build is what actually executed `/maintain` and `/finish-session` here, and its results were reported as the repo's verification. Every gate it ran passed. The check compares the running distribution against the vendored `pyproject.toml`, prints one line to stderr naming both versions and the reinstall command, and is silent when they match, when there is no vendored copy (the other-repos case, which is the tool's whole distribution story), and on any parse failure — it runs before every command and must never be why one fails.
- The self-check shipped broken for the length of one test run, and its own tests could not see it. An inline `from pathlib import Path` in the `init` branch made the module-level name function-local for all of `main()`, so every command raised `UnboundLocalError` while all six unit tests passed — they called the helper directly and none went through dispatch. Caught by running the binary, not by the suite. There is now a wiring test that drives `main()`, verified to redden when the shadowing import is reintroduced.
...
### fixed
- **`probe.py` handed the bearer token to whatever host a redirect named.** urllib's redirect handler copies every header onto the redirected request, so `--api-key sk-... ` against a server answering 302 sent `Authorization: Bearer sk-...` to the Location host — on both `/v1/models` and `/v1/capabilities` — and still exited 0 with a matrix, so nothing surfaced. Reproduced against a live pair of loopback servers before and after. The probe now opens through a handler that strips `Authorization` when the redirect leaves the origin, keeping it for same-origin rewrites because an http path rewrite on the same server is a real deployment and breaking it pushes people back to no auth at all. The script's docstring already promised the key is never printed; forwarding it to a third party was the same promise broken more quietly.
- **Three more defects in the code that shipped one release ago, two of them regressions introduced by the release that fixed the last set.** `fetch` caught only `(URLError, OSError)`, so `http.client.HTTPException` escaped as a traceback — `BadStatusLine` when something that is not an HTTP server answers on the port, `IncompleteRead` on a body cut off mid-load. Worse, 0.2.0 had narrowed the `/v1/capabilities` guard from `except Exception` to `except Unreadable`, converting a failure 0.1.0 survived into a crash *after* a good roster was already in hand; the test written to guarantee "caps is best-effort" only exercised the 404 path, which is wrapped, and so could not see the regression. The guard is deliberately broad again. And the 401 hint said "pass `--api-key` or set `HEYLOOK_API_KEY`" without checking whether a key had just been sent and rejected — the identical defect 0.2.0 fixed one arm over, where 0.1.0 answered "start the server" to a server that was already running.
- **One unusable row no longer condemns the roster.** A `/v1/models` row without an `id` raised and exited 1, which per the skill body means "could not read the server" — sending the operator to check ports and hiding the models they could have used. Malformed rows are now skipped with a count on stderr; exit 1 is reserved for a roster with nothing usable in it, which is the only case where there is no model to pick.
- **`Retry-After: 0` turned the backoff recipe into a hot loop.** `min(retry_after, 30)` slept zero for a legal zero value, firing all five attempts back-to-back at a server that had just said it was saturated — where the exponential fallback it replaced always slept at least a second. Floored at 1s.
...
### fixed
- **`heylook-provider` 0.2.0 → 0.3.0 — a code review of the upstream 1.79.39 work found that half of what 0.2.0 newly asserted was wrong, and the skill inherited it.** The biggest: 0.2.0 listed `error` as a fourth `stop_reason` on the stated grounds that the server set it on the non-streaming failure path. It never did — `MessageResponse` has one construction site, fed only by a mapper that cannot produce that value, and a non-streaming failure raises rather than returning a response. Upstream removed the member in 1.79.40; the skill no longer documents a branch that cannot be entered. Also gone: 0.2.0's claim that its list of differences was **closed**, which was false in four ways it did not know about (thinking blocks carry no `signature`, there is no `stop_sequence` field, `message_start` omits both, and `logprobs` can be a non-streaming content block) plus a fifth found later (`message_start.usage.input_tokens` is structurally 0, because the event is emitted before the first chunk is absorbed). The list now says it is hand-maintained, has been wrong, and loses to `/openapi.json`.
- **The reference contradicted itself about the same payload and both halves were true.** One line said a media block missing `data` and `url` is silently dropped; another said it is a 422. It depends: no `source`/`source_type` at all fails validation, while a `source_type` with no payload validates and is then dropped during conversion — the request succeeds, the text survives, and the model never sees the image. Stating the boundary is more useful than deleting either half, and the failure with no status code is the one worth warning about.
- **The streaming grammar documented an event that cannot exist.** Showing the text and thinking variants as two `data:` lines under one `event:` line reads, in SSE, as a single event carrying two JSON objects — and the `_sse` reader shipped in the same file yields once per `data:` line, so a client implementing the documented grammar would emit two `content_block_start` events per frame. The variants are now annotated prose, not a second data line.
- **Both streaming recipes treated every non-`thinking_delta` as a `text_delta`.** Anthropic emits `signature_delta` on thinking blocks; under that else branch TypeScript appended the literal `"undefined"` to the user-visible answer and Python raised `KeyError`. Survivable when the skill said "here are four traps"; not once its thesis became "assume Anthropic's spec is the answer". Both now key on the types they handle and ignore the rest. The Python thinking fallback also used `.get(key, default)`, which triggers on absence — an explicit JSON `null` is present-but-`None` and raised `TypeError` on `+=`; it now uses an `or` chain matching the TypeScript `??`. Both were re-executed against a stream carrying a null `thinking` and a `signature_delta`.
- The plugin README's skills-table row still sold "the divergences from the APIs heylook resembles" — the framing 0.2.0 retired one section below it, and the row is what a plugin browser renders. Also picked up: the nested `source` form is now visible in the generated JSON Schema (upstream declared it as a real field), and `/v1/conversations` accepts only that shape, which is the concrete reason to prefer it.
