...
- **Nothing else reads it** -- a database is the store. `readwise-reader` mirrors a remote SaaS with FTS indexes and staged reconciliation: there is no local file that could be authoritative, because the truth is on someone else's server. `agent-state`'s **run lineage** qualifies for the same reason -- parent/child trees with staging and commit semantics are instrumented facts about a run, and nothing writes them to a file.

  Its **watermark** table does not, and the distinction is worth keeping because it was got wrong here first. `.skill-maintainer/state/upstream_hashes.json` holds current values and `changes.jsonl` holds the history of what changed and when; between them they carry what the table normalizes. Applying the test properly retires that half of the schema rather than blessing it. A worked example beats a principle: run the test on your own units before citing them as exemplars.
