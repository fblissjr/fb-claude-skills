...
from . import auth, client as call_mod, ledger, media, recipes, runs
...
def cmd_ask(args: argparse.Namespace) -> int:
    ...
    for att in attachments:
        for pattern in cfg.sensitive_paths:
            if att.path.match(pattern):
                return _fail(
                    f"{att.path} matches sensitive path pattern {pattern!r}. "
    ...
    run.write_request(call_mod.redact_for_record(request, attachments))
    ...
    run.write_response(result.text)
    run.write_usage(result.usage)
    if result.structured is not None:
        run.write_structured(result.structured)
    if result.interaction_id:
        run.write_interaction_id(result.interaction_id)
    ...
    return 0 if result.status == call_mod.TERMINAL_OK else 2
