...
1b. **One changelog, at the repo root.** No unit gets its own `CHANGELOG.md` — `apps/readwise-reader` was the only first-party exception and it drifted five versions behind its own `pyproject.toml` before anyone noticed (removed 2026-07-26). The general rule this came from, and the one to apply to the next duplicated field: **a copy earns its place only if it has a consumer other than the check that confirms it is a copy.** `plugin.json` and `marketplace.json` versions each have a real consumer and are machine-checked against one another; SKILL.md's `metadata.version` and per-unit changelog headings had none, and both are gone. The two-question form, settled 2026-08-04 when the repo-local `control-builder` agent was retired in favor of the copy the postmortem plugin ships: **name the copy's consumer, and name what watches the pair — if either answer is "nothing", delete the copy or demote it to data the shipped mechanism cites at dispatch.** This applies with full force to local copies of components this repo's own plugins ship (agents, skills, procedure prose): an unwatched local variant splits dogfooding from what installs run, so defects in the shipped copy stop being noticed here first. Every legitimate pair in this repo passes the test one of three ways — a mechanical mirror (invariant 3's best_practices hook), a designed handoff where the local copy is authoritative and the plugin self-silences (invariant 6 / dev-conventions ground coverage), or data-vs-mechanism separation (local evidence quoted into a dispatch of the shipped mechanism, never welded to a second copy of it).
...
| The claim-audit skill (diff prose audited by execution, instrument-yield routing) — SHIPPED 2026-08-04 as `skills/claim-audit/`; the doc is the design record | [docs/internals/claim_audit_design.md](docs/internals/claim_audit_design.md) |
| The control-audit skill (census + adversarial live-fire over hooks, validators, reminders) and the adversarial-verify primitive it dispatches to — SHIPPED 2026-08-04 in the postmortem plugin (0.7.0/0.8.0); the doc is the design record | [docs/internals/control_audit_design.md](docs/internals/control_audit_design.md) |
